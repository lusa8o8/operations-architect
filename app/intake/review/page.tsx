...
type Phase = 'edit' | 'checking' | 'review_warnings' | 'confirming' | 'confirmed'
...
export default function ReviewPage() {
  ...
  function removeStage(i: number) {
    setRemovedStages(p => [...p, stages[i]])
    setStages(p => p.filter((_, j) => j !== i))
  }
  function undoRemoveStage() {
    const last = removedStages[removedStages.length - 1]
    if (!last) return
    setStages(p => [...p, last].sort((a, b) => a.order - b.order))
    setRemovedStages(p => p.slice(0, -1))
  }
  function removeRole(i: number) {
    setRemovedRoles(p => [...p, roles[i]])
  ...
  function undoRemoveRole() {
    ...
    if (!last) return
    setRoles(p => [...p, last])
    setRemovedRoles(p => p.slice(0, -1))
  }
  function removeTrigger(i: number) {
    setRemovedTriggers(p => [...p, triggers[i]])
    setTriggers(p => p.filter((_, j) => j !== i))
  }
  function undoRemoveTrigger() {
    const last = removedTriggers[removedTriggers.length - 1]
    if (!last) return
    setTriggers(p => [...p, last])
    setRemovedTriggers(p => p.slice(0, -1))
  }
  function startEditStage(i: number) {
    setEditingStage(i)
    setEditingText(stages[i].name)
  }
  function saveEditStage(i: number) {
    setStages(p => p.map((s, j) => j === i ? { ...s, name: editingText } : s))
    setEditingStage(null)
    setEditingText('')
  }
  function addStage() {
    if (!newStageName.trim()) return
    setStages(p => [...p, { name: newStageName.trim(), description: '', order: p.length + 1 }])
    setNewStageName('')
    setShowAddStage(false)
  }
  function addRole() {
    if (!newRoleName.trim()) return
    setRoles(p => [...p, { name: newRoleName.trim(), type: 'human' }])
    setNewRoleName('')
    setShowAddRole(false)
  }
  function addTrigger() {
    if (!newTriggerName.trim()) return
    setTriggers(p => [...p, { name: newTriggerName.trim(), description: '' }])
    setNewTriggerName('')
  ...
  async function handleSubmitForReview() {
    ...
    const res = await fetch('/api/intake/review', {
      ...
      body: JSON.stringify({ sessionId, confirmedStages: stages, confirmedRoles: roles, confirmedTriggers: triggers })
    })
    const data = await res.json()
    setConsistency(data)
    setPhase('review_warnings')
  }
  ...
  const s = { minHeight: '100vh', backgroundColor: '#0A0A0F', color: '#E8E8F0', fontFamily: 'monospace', padding: '48px 32px' }
  ...
  if (phase === 'checking') return (
    <div style={{ ...s, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ maxWidth: '600px', width: '100%' }}>
        <span style={{ color: '#4DFFA0', fontSize: '14px' }}>OA</span>
        <h2 style={{ fontSize: '20px', margin: '24px 0 8px', fontFamily: 'serif', fontWeight: 400 }}>Checking your model...</h2>
        <p style={{ color: '#6B6B8A', fontSize: '14px' }}>The system is reviewing your changes for operational consistency.</p>
      </div>
    </div>
  )

  if ((phase === 'review_warnings' || phase === 'confirming') && consistency) return (
    <div style={s}>
      <div style={{ maxWidth: '760px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '48px' }}>
          <span style={{ color: '#4DFFA0', fontSize: '14px' }}>OA</span>
          <button onClick={() => setPhase('edit')} style={{ background: 'none', border: 'none', color: '#6B6B8A', fontSize: '13px', cursor: 'pointer' }}>← Back to edit</button>
        </div>

        <h1 style={{ fontSize: '24px', marginBottom: '8px' }}>Final review.</h1>
        <p style={{ color: '#6B6B8A', fontSize: '14px', marginBottom: '32px' }}>{consistency.summary}</p>

        {consistency.changes_detected.length > 0 && (
          <div style={{ marginBottom: '32px', padding: '20px', border: '1px solid #2A2A38', borderRadius: '8px' }}>
            <p style={{ color: '#6B6B8A', fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '12px' }}>Changes from original</p>
            {consistency.changes_detected.map((c, i) => (
              <p key={i} style={{ fontSize: '13px', color: '#E8E8F0', marginBottom: '6px' }}>· {c}</p>
            ))}
          </div>
        )}
        ...
        {consistency.warnings.length > 0 && (
          <div style={{ marginBottom: '32px' }}>
            <p style={{ color: '#6B6B8A', fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '12px' }}>Flags</p>
            {consistency.warnings.map((w, i) => (
              <div key={i} style={{ padding: '16px', border: `1px solid ${w.severity === 'high' ? '#FF6B6B44' : w.severity === 'medium' ? '#FFB84444' : '#2A2A38'}`, borderRadius: '8px', marginBottom: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                  {tag(w.severity === 'high' ? '#FF6B6B' : w.severity === 'medium' ? '#FFB844' : '#6B6B8A', w.severity)}
                  <p style={{ fontSize: '13px', fontWeight: 500 }}>{w.message}</p>
                </div>
                <p style={{ fontSize: '12px', color: '#6B6B8A' }}>→ {w.suggestion}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {consistency.warnings.length === 0 && (
          <div style={{ marginBottom: '32px', padding: '16px', border: '1px solid #4DFFA044', borderRadius: '8px' }}>
            <p style={{ color: '#4DFFA0', fontSize: '13px' }}>✓ No issues detected. Your model is operationally consistent.</p>
          </div>
        )}
        ...
        <div style={{ borderTop: '1px solid #2A2A38', paddingTop: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <button onClick={() => setPhase('edit')}
            style={{ background: 'none', border: '1px solid #2A2A38', color: '#6B6B8A', padding: '12px 24px', borderRadius: '6px', fontSize: '14px', cursor: 'pointer' }}>
            ← Go back and edit
          </button>
          <button onClick={handleFinalConfirm}
            style={{ padding: '14px 32px', background: '#4DFFA0', color: '#0A0A0F', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}>
            {phase === 'confirming' ? 'Locking...' : 'Lock pipeline →'}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={s}>
      <div style={{ maxWidth: '760px', margin: '0 auto' }}>
        ...
        <p style={{ color: '#6B6B8A', fontSize: '14px', marginBottom: '40px' }}>Remove anything wrong, edit what's unclear, then submit for final review.</p>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          {removedStages.length > 0 && (
            <button onClick={undoRemoveStage} style={{ background: 'none', border: 'none', color: '#4DFFA0', fontSize: '12px', cursor: 'pointer', marginBottom: '8px', display: 'block' }}>
              ↩ Undo remove "{removedStages[removedStages.length - 1].name}"
            </button>
          )}
          {showAddStage ? (
          ...
          {stages.length === 0 && <p style={{ color: '#FF6B6B', fontSize: '13px', marginTop: '8px' }}>No stages — add at least one before submitting.</p>}
        </div>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          {removedRoles.length > 0 && (
            <button onClick={undoRemoveRole} style={{ background: 'none', border: 'none', color: '#4DFFA0', fontSize: '12px', cursor: 'pointer', marginBottom: '8px', display: 'block' }}>
              ↩ Undo remove "{removedRoles[removedRoles.length - 1].name}"
            </button>
          )}
          {showAddRole ? (
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          {removedTriggers.length > 0 && (
            <button onClick={undoRemoveTrigger} style={{ background: 'none', border: 'none', color: '#4DFFA0', fontSize: '12px', cursor: 'pointer', marginBottom: '8px', display: 'block' }}>
              ↩ Undo remove "{removedTriggers[removedTriggers.length - 1].name}"
            </button>
          )}
          {showAddTrigger ? (
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
        ...
        <div style={{ borderTop: '1px solid #2A2A38', paddingTop: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p style={{ color: '#6B6B8A', fontSize: '13px' }}>{stages.length} stages · {roles.length} roles · {triggers.length} triggers</p>
          <button onClick={handleSubmitForReview} disabled={stages.length === 0}
